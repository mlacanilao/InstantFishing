...
public class Patcher
{
    ...
    public static void AI_FishMakefish(Chara c, ref Thing __result)
    {
        AI_FishPatch.MakefishPostfix(c: c, __result: ref __result);
    }
    ...
    [HarmonyPrefix]
    [HarmonyPatch(declaringType: typeof(Point), methodName: nameof(Point.PlaySound), argumentTypes: new[] { typeof(string), typeof(bool), typeof(float), typeof(bool) })]
    public static bool PointPlaySound(string id)
    {
        return CardPatch.PointPlaySoundPrefix(id: id);
    }
    ...
    public static void LayerSleepAdvancePrefix(LayerSleep __instance, ref LayerSleepPatch.SleepAdvanceState __state)
    {
        LayerSleepPatch.AdvancePrefix(__instance: __instance, __state: ref __state);
    }

    [HarmonyPostfix]
    [HarmonyPatch(declaringType: typeof(LayerSleep), methodName: nameof(LayerSleep.Advance))]
    public static void LayerSleepAdvance(LayerSleepPatch.SleepAdvanceState __state)
    {
        LayerSleepPatch.AdvancePostfix(__state: __state);
    }
    
    [HarmonyPrefix]
    [HarmonyPatch(declaringType: typeof(StatsStamina), methodName: nameof(StatsStamina.Mod))]
    public static bool StatsStaminaMod(int a)
    {
        return StatsStaminaPatch.ModPrefix(a: a);
    }
}
